...
- ~~**i18n de mensajes de error/validación**~~ ✅ PATRÓN IMPLANTADO 30 jul (decisión #84, commit `927a755`): códigos estables (`lib/errors.ts` + namespace `Errors`) en invite/orgs/onboarding/Zod + `useZodMessage()` en los 9 sitios RHF; de paso se cerraron 2 fugas de mensajes internos al navegador. **Resto mecánico delegable a Sonnet:** 5 actions de admin con prosa `auth.*` (claves ya en catálogo: `validate-ground-truth`, `train-rp-models`, `notify-discord`, `calibration`), mensajes custom de Zod restantes ("Máx N caracteres", "Match # debe ser positivo" — ya fluyen por el helper, es puro catálogo+swap), y errorMap global de Zod para los defaults ingleses.
